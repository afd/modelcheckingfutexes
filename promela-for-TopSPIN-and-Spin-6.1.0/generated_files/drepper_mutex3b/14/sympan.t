...
{	Trans *T;
	...
	T = trans[ 2][16] = settr(136,2,0,0,0,"ATOMIC", 0, 2, 0);
	T->nxt	= settr(136,2,1,0,0,"ATOMIC", 0, 2, 0);
	trans[2][1]	= settr(121,2,2,3,3,"(run Thread())", 0, 2, 0);
	trans[2][2]	= settr(122,2,3,4,4,"(run Thread())", 0, 2, 0);
	trans[2][3]	= settr(123,2,4,5,5,"(run Thread())", 0, 2, 0);
	trans[2][4]	= settr(124,2,5,6,6,"(run Thread())", 0, 2, 0);
	trans[2][5]	= settr(125,2,6,7,7,"(run Thread())", 0, 2, 0);
	trans[2][6]	= settr(126,2,7,8,8,"(run Thread())", 0, 2, 0);
	trans[2][7]	= settr(127,2,8,9,9,"(run Thread())", 0, 2, 0);
	trans[2][8]	= settr(128,2,9,10,10,"(run Thread())", 0, 2, 0);
	trans[2][9]	= settr(129,2,10,11,11,"(run Thread())", 0, 2, 0);
	trans[2][10]	= settr(130,2,11,12,12,"(run Thread())", 0, 2, 0);
	trans[2][11]	= settr(131,2,12,13,13,"(run Thread())", 0, 2, 0);
	trans[2][12]	= settr(132,2,13,14,14,"(run Thread())", 0, 2, 0);
	trans[2][13]	= settr(133,2,14,15,15,"(run Thread())", 0, 2, 0);
	trans[2][14]	= settr(134,2,15,16,16,"(run Thread())", 0, 2, 0);
	trans[2][15]	= settr(135,0,17,17,17,"(run Monitor())", 0, 2, 0);
	trans[2][17]	= settr(137,0,0,18,18,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(117,4,4,19,19,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(120,0,0,20,20,"-end-", 0, 3500, 0);
	...
	T = T->nxt	= settr(113,0,50,0,0,"DO", 0, 2, 0);
...
/*->*/	trans[0][40]	= settr(39,32,42,31,31,"D_STEP", 1, 2, 0);
	...
	trans[0][52]	= settr(51,0,111,34,34,"num_threads_in_cs = (num_threads_in_cs-1)", 1, 2, 0);
	T = trans[ 0][111] = settr(110,0,0,0,0,"sub-sequence", 0, 2, 0);
	T->nxt	= settr(110,0,58,0,0,"sub-sequence", 0, 2, 0);
/*->*/	trans[0][58]	= settr(57,32,109,35,35,"D_STEP", 1, 2, 0);
	T = trans[0][109] = settr(108,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(108,0,61,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(108,0,108,0,0,"IF", 0, 2, 0);
/*->*/	trans[0][61]	= settr(60,32,105,36,36,"D_STEP", 0, 2, 0);
	...
	T->nxt	= settr(103,2,62,0,0,"ATOMIC", 1, 2, 0);
	trans[0][62]	= settr(61,2,99,37,37,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 63 -> 0,99 */
	reached0[63] = 1;
	...
	T = T->nxt	= settr(98,2,64,0,0,"DO", 1, 2, 0);
	    T->nxt	= settr(98,2,66,0,0,"DO", 1, 2, 0);
	trans[0][64]	= settr(63,0,114,38,38,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 102 -> 114,0 */
	reached0[102] = 1;
	trans[0][65]	= settr(64,2,102,1,0,"goto :b2", 1, 2, 0); /* m: 102 -> 0,114 */
	reached0[102] = 1;
	trans[0][66]	= settr(65,2,95,2,0,"else", 1, 2, 0);
	T = trans[0][95] = settr(94,2,0,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(94,2,67,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(94,2,69,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(94,2,71,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(94,2,73,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(94,2,75,0,0,"IF", 1, 2, 0);
	...
	trans[0][67]	= settr(66,2,99,39,39,"(futex.wait[1])", 1, 2, 0); /* m: 68 -> 99,0 */
	reached0[68] = 1;
	trans[0][68]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][96]	= settr(95,2,97,1,0,".(goto)", 1, 2, 0); /* m: 97 -> 0,99 */
	reached0[97] = 1;
	trans[0][69]	= settr(68,2,99,40,40,"(futex.wait[2])", 1, 2, 0); /* m: 70 -> 99,0 */
	reached0[70] = 1;
	trans[0][70]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][71]	= settr(70,2,99,41,41,"(futex.wait[3])", 1, 2, 0); /* m: 72 -> 99,0 */
	reached0[72] = 1;
	trans[0][72]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][73]	= settr(72,2,99,42,42,"(futex.wait[4])", 1, 2, 0); /* m: 74 -> 99,0 */
	reached0[74] = 1;
	trans[0][74]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][75]	= settr(74,2,99,43,43,"(futex.wait[5])", 1, 2, 0); /* m: 76 -> 99,0 */
	reached0[76] = 1;
	trans[0][76]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][77]	= settr(76,2,99,44,44,"(futex.wait[6])", 1, 2, 0); /* m: 78 -> 99,0 */
	reached0[78] = 1;
	trans[0][78]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][79]	= settr(78,2,99,45,45,"(futex.wait[7])", 1, 2, 0); /* m: 80 -> 99,0 */
	reached0[80] = 1;
	trans[0][80]	= settr(0,0,0,0,0,"futex.wait[7] = 0",0,0,0);
	trans[0][81]	= settr(80,2,99,46,46,"(futex.wait[8])", 1, 2, 0); /* m: 82 -> 99,0 */
	reached0[82] = 1;
	trans[0][82]	= settr(0,0,0,0,0,"futex.wait[8] = 0",0,0,0);
	trans[0][83]	= settr(82,2,99,47,47,"(futex.wait[9])", 1, 2, 0); /* m: 84 -> 99,0 */
	reached0[84] = 1;
	trans[0][84]	= settr(0,0,0,0,0,"futex.wait[9] = 0",0,0,0);
	trans[0][85]	= settr(84,2,99,48,48,"(futex.wait[10])", 1, 2, 0); /* m: 86 -> 99,0 */
	reached0[86] = 1;
	trans[0][86]	= settr(0,0,0,0,0,"futex.wait[10] = 0",0,0,0);
	trans[0][87]	= settr(86,2,99,49,49,"(futex.wait[11])", 1, 2, 0); /* m: 88 -> 99,0 */
	reached0[88] = 1;
	trans[0][88]	= settr(0,0,0,0,0,"futex.wait[11] = 0",0,0,0);
	trans[0][89]	= settr(88,2,99,50,50,"(futex.wait[12])", 1, 2, 0); /* m: 90 -> 99,0 */
	reached0[90] = 1;
	trans[0][90]	= settr(0,0,0,0,0,"futex.wait[12] = 0",0,0,0);
	trans[0][91]	= settr(90,2,99,51,51,"(futex.wait[13])", 1, 2, 0); /* m: 92 -> 99,0 */
	reached0[92] = 1;
	trans[0][92]	= settr(0,0,0,0,0,"futex.wait[13] = 0",0,0,0);
	trans[0][93]	= settr(92,2,99,52,52,"(futex.wait[14])", 1, 2, 0); /* m: 94 -> 99,0 */
	reached0[94] = 1;
	trans[0][94]	= settr(0,0,0,0,0,"futex.wait[14] = 0",0,0,0);
	trans[0][97]	= settr(96,2,99,53,53,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 98 -> 0,99 */
	reached0[98] = 1;
	trans[0][98]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][101]	= settr(100,2,102,1,0,"break", 1, 2, 0);
	trans[0][102]	= settr(101,0,114,54,54,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 103 -> 0,114 */
	reached0[103] = 1;
	trans[0][103]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][110]	= settr(109,0,114,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][108]	= settr(107,32,114,55,55,"D_STEP", 0, 2, 0);
	trans[0][112]	= settr(111,0,117,56,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][113]	= settr(112,0,117,1,0,"goto :b0", 0, 2, 0);
	trans[0][116]	= settr(115,0,117,1,0,"break", 0, 2, 0);
	trans[0][117]	= settr(116,0,0,57,57,"-end-", 0, 3500, 0);
	/* np_ demon: */
