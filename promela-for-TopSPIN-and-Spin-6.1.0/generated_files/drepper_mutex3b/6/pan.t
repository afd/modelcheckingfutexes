...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(106,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(107,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(108,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(109,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= /* c */ settr(110,2,7,3,3,"(run Thread())", 0, 2, 0);
	trans[2][7]	= settr(111,0,9,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][9]	= settr(113,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(101,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(104,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][61]	= settr(60,32,89,23,23,"D_STEP", 0, 2, 0);
	...
	trans[0][62]	= settr(61,2,83,24,24,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 63 -> 0,83 */
	reached0[63] = 1;
	...
	trans[0][64]	= settr(63,0,98,25,25,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 86 -> 98,0 */
	reached0[86] = 1;
	...
	trans[0][67]	= settr(66,2,83,26,26,"(futex.wait[1])", 1, 2, 0); /* m: 68 -> 83,0 */
	reached0[68] = 1;
	trans[0][68]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][80]	= settr(79,2,81,1,0,".(goto)", 1, 2, 0); /* m: 81 -> 0,83 */
	reached0[81] = 1;
	trans[0][69]	= settr(68,2,83,27,27,"(futex.wait[2])", 1, 2, 0); /* m: 70 -> 83,0 */
	reached0[70] = 1;
	trans[0][70]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][71]	= settr(70,2,83,28,28,"(futex.wait[3])", 1, 2, 0); /* m: 72 -> 83,0 */
	reached0[72] = 1;
	trans[0][72]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][73]	= settr(72,2,83,29,29,"(futex.wait[4])", 1, 2, 0); /* m: 74 -> 83,0 */
	reached0[74] = 1;
	trans[0][74]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][75]	= settr(74,2,83,30,30,"(futex.wait[5])", 1, 2, 0); /* m: 76 -> 83,0 */
	reached0[76] = 1;
	trans[0][76]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][77]	= settr(76,2,83,31,31,"(futex.wait[6])", 1, 2, 0); /* m: 78 -> 83,0 */
	reached0[78] = 1;
	trans[0][78]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][81]	= settr(80,2,83,32,32,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 82 -> 0,83 */
	reached0[82] = 1;
	trans[0][82]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][85]	= settr(84,2,86,1,0,"break", 1, 2, 0);
	trans[0][86]	= settr(85,0,98,33,33,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 87 -> 0,98 */
	reached0[87] = 1;
	trans[0][87]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][94]	= settr(93,0,98,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][92]	= settr(91,32,98,34,34,"D_STEP", 0, 2, 0);
	trans[0][96]	= settr(95,0,101,35,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][97]	= settr(96,0,101,1,0,"goto :b0", 0, 2, 0);
	trans[0][100]	= settr(99,0,101,1,0,"break", 0, 2, 0);
	trans[0][101]	= settr(100,0,0,36,36,"-end-", 0, 3500, 0);
	/* np_ demon: */
