...
{	Trans *T;
	...
	trans[2][2]	= settr(107,2,3,4,4,"(run Thread())", 0, 2, 0);
	trans[2][3]	= settr(108,2,4,5,5,"(run Thread())", 0, 2, 0);
	trans[2][4]	= settr(109,2,5,6,6,"(run Thread())", 0, 2, 0);
	trans[2][5]	= settr(110,2,6,7,7,"(run Thread())", 0, 2, 0);
	trans[2][6]	= settr(111,2,7,8,8,"(run Thread())", 0, 2, 0);
	trans[2][7]	= settr(112,0,9,9,9,"(run Monitor())", 0, 2, 0);
	trans[2][9]	= settr(114,0,0,10,10,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(102,4,4,11,11,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(105,0,0,12,12,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][62]	= settr(61,32,90,28,28,"D_STEP", 1, 2, 0);
	...
	trans[0][63]	= settr(62,2,84,29,29,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 64 -> 0,84 */
	reached0[64] = 1;
	...
	trans[0][65]	= settr(64,0,99,30,30,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 87 -> 99,0 */
	reached0[87] = 1;
	...
	trans[0][68]	= settr(67,2,84,31,31,"(futex.wait[1])", 1, 2, 0); /* m: 69 -> 84,0 */
	reached0[69] = 1;
	trans[0][69]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][81]	= settr(80,2,82,1,0,".(goto)", 1, 2, 0); /* m: 82 -> 0,84 */
	reached0[82] = 1;
	trans[0][70]	= settr(69,2,84,32,32,"(futex.wait[2])", 1, 2, 0); /* m: 71 -> 84,0 */
	reached0[71] = 1;
	trans[0][71]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][72]	= settr(71,2,84,33,33,"(futex.wait[3])", 1, 2, 0); /* m: 73 -> 84,0 */
	reached0[73] = 1;
	trans[0][73]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][74]	= settr(73,2,84,34,34,"(futex.wait[4])", 1, 2, 0); /* m: 75 -> 84,0 */
	reached0[75] = 1;
	trans[0][75]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][76]	= settr(75,2,84,35,35,"(futex.wait[5])", 1, 2, 0); /* m: 77 -> 84,0 */
	reached0[77] = 1;
	trans[0][77]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][78]	= settr(77,2,84,36,36,"(futex.wait[6])", 1, 2, 0); /* m: 79 -> 84,0 */
	reached0[79] = 1;
	trans[0][79]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][82]	= settr(81,2,84,37,37,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 83 -> 0,84 */
	reached0[83] = 1;
	trans[0][83]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][86]	= settr(85,2,87,1,0,"break", 1, 2, 0);
	trans[0][87]	= settr(86,0,99,38,38,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 88 -> 0,99 */
	reached0[88] = 1;
	trans[0][88]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][95]	= settr(94,0,99,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][93]	= settr(92,32,99,39,39,"D_STEP", 0, 2, 0);
	trans[0][97]	= settr(96,0,102,40,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][98]	= settr(97,0,102,1,0,"goto :b0", 0, 2, 0);
	trans[0][101]	= settr(100,0,102,1,0,"break", 0, 2, 0);
	trans[0][102]	= settr(101,0,0,41,41,"-end-", 0, 3500, 0);
	/* np_ demon: */
