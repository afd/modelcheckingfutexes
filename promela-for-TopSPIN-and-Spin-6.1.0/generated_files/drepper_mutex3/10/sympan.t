...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(115,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(116,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(117,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(118,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= /* c */ settr(119,2,7,3,3,"(run Thread())", 0, 2, 0);
	trans[2][7]	= /* c */ settr(120,2,8,3,3,"(run Thread())", 0, 2, 0);
	trans[2][8]	= /* c */ settr(121,2,9,3,3,"(run Thread())", 0, 2, 0);
	trans[2][9]	= /* c */ settr(122,2,10,3,3,"(run Thread())", 0, 2, 0);
	trans[2][10]	= /* c */ settr(123,2,11,3,3,"(run Thread())", 0, 2, 0);
	trans[2][11]	= settr(124,0,13,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][13]	= settr(126,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(110,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(113,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][62]	= settr(61,32,98,23,23,"D_STEP", 1, 2, 0);
	...
	trans[0][63]	= settr(62,2,92,24,24,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 64 -> 0,92 */
	reached0[64] = 1;
	...
	trans[0][65]	= settr(64,0,107,25,25,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 95 -> 107,0 */
	reached0[95] = 1;
	...
	trans[0][68]	= settr(67,2,92,26,26,"(futex.wait[1])", 1, 2, 0); /* m: 69 -> 92,0 */
	reached0[69] = 1;
	trans[0][69]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][89]	= settr(88,2,90,1,0,".(goto)", 1, 2, 0); /* m: 90 -> 0,92 */
	reached0[90] = 1;
	trans[0][70]	= settr(69,2,92,27,27,"(futex.wait[2])", 1, 2, 0); /* m: 71 -> 92,0 */
	reached0[71] = 1;
	trans[0][71]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][72]	= settr(71,2,92,28,28,"(futex.wait[3])", 1, 2, 0); /* m: 73 -> 92,0 */
	reached0[73] = 1;
	trans[0][73]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][74]	= settr(73,2,92,29,29,"(futex.wait[4])", 1, 2, 0); /* m: 75 -> 92,0 */
	reached0[75] = 1;
	trans[0][75]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][76]	= settr(75,2,92,30,30,"(futex.wait[5])", 1, 2, 0); /* m: 77 -> 92,0 */
	reached0[77] = 1;
	trans[0][77]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][78]	= settr(77,2,92,31,31,"(futex.wait[6])", 1, 2, 0); /* m: 79 -> 92,0 */
	reached0[79] = 1;
	trans[0][79]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][80]	= settr(79,2,92,32,32,"(futex.wait[7])", 1, 2, 0); /* m: 81 -> 92,0 */
	reached0[81] = 1;
	trans[0][81]	= settr(0,0,0,0,0,"futex.wait[7] = 0",0,0,0);
	trans[0][82]	= settr(81,2,92,33,33,"(futex.wait[8])", 1, 2, 0); /* m: 83 -> 92,0 */
	reached0[83] = 1;
	trans[0][83]	= settr(0,0,0,0,0,"futex.wait[8] = 0",0,0,0);
	trans[0][84]	= settr(83,2,92,34,34,"(futex.wait[9])", 1, 2, 0); /* m: 85 -> 92,0 */
	reached0[85] = 1;
	trans[0][85]	= settr(0,0,0,0,0,"futex.wait[9] = 0",0,0,0);
	trans[0][86]	= settr(85,2,92,35,35,"(futex.wait[10])", 1, 2, 0); /* m: 87 -> 92,0 */
	reached0[87] = 1;
	trans[0][87]	= settr(0,0,0,0,0,"futex.wait[10] = 0",0,0,0);
	trans[0][90]	= settr(89,2,92,36,36,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 91 -> 0,92 */
	reached0[91] = 1;
	trans[0][91]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][94]	= settr(93,2,95,1,0,"break", 1, 2, 0);
	trans[0][95]	= settr(94,0,107,37,37,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 96 -> 0,107 */
	reached0[96] = 1;
	trans[0][96]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][103]	= settr(102,0,107,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][101]	= settr(100,32,107,38,38,"D_STEP", 0, 2, 0);
	trans[0][105]	= settr(104,0,110,39,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][106]	= settr(105,0,110,1,0,"goto :b0", 0, 2, 0);
	trans[0][109]	= settr(108,0,110,1,0,"break", 0, 2, 0);
	trans[0][110]	= settr(109,0,0,40,40,"-end-", 0, 3500, 0);
	/* np_ demon: */
