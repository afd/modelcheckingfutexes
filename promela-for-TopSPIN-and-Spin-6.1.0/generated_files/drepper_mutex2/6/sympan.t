...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(116,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(117,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(118,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(119,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= /* c */ settr(120,2,7,3,3,"(run Thread())", 0, 2, 0);
	trans[2][7]	= settr(121,0,9,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][9]	= settr(123,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(111,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(114,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][71]	= settr(70,32,99,28,28,"D_STEP", 1, 2, 0);
	...
	trans[0][72]	= settr(71,2,93,29,29,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 73 -> 0,93 */
	reached0[73] = 1;
	...
	trans[0][74]	= settr(73,0,108,30,30,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 96 -> 108,0 */
	reached0[96] = 1;
	...
	trans[0][77]	= settr(76,2,93,31,31,"(futex.wait[1])", 1, 2, 0); /* m: 78 -> 93,0 */
	reached0[78] = 1;
	trans[0][78]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][90]	= settr(89,2,91,1,0,".(goto)", 1, 2, 0); /* m: 91 -> 0,93 */
	reached0[91] = 1;
	trans[0][79]	= settr(78,2,93,32,32,"(futex.wait[2])", 1, 2, 0); /* m: 80 -> 93,0 */
	reached0[80] = 1;
	trans[0][80]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][81]	= settr(80,2,93,33,33,"(futex.wait[3])", 1, 2, 0); /* m: 82 -> 93,0 */
	reached0[82] = 1;
	trans[0][82]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][83]	= settr(82,2,93,34,34,"(futex.wait[4])", 1, 2, 0); /* m: 84 -> 93,0 */
	reached0[84] = 1;
	trans[0][84]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][85]	= settr(84,2,93,35,35,"(futex.wait[5])", 1, 2, 0); /* m: 86 -> 93,0 */
	reached0[86] = 1;
	trans[0][86]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][87]	= settr(86,2,93,36,36,"(futex.wait[6])", 1, 2, 0); /* m: 88 -> 93,0 */
	reached0[88] = 1;
	trans[0][88]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][91]	= settr(90,2,93,37,37,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 92 -> 0,93 */
	reached0[92] = 1;
	trans[0][92]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][95]	= settr(94,2,96,1,0,"break", 1, 2, 0);
	trans[0][96]	= settr(95,0,108,38,38,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 97 -> 0,108 */
	reached0[97] = 1;
	trans[0][97]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][104]	= settr(103,0,108,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][102]	= settr(101,32,108,39,39,"D_STEP", 0, 2, 0);
	trans[0][106]	= settr(105,0,111,40,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][107]	= settr(106,0,111,1,0,"goto :b0", 0, 2, 0);
	trans[0][110]	= settr(109,0,111,1,0,"break", 0, 2, 0);
	trans[0][111]	= settr(110,0,0,41,41,"-end-", 0, 3500, 0);
	/* np_ demon: */
