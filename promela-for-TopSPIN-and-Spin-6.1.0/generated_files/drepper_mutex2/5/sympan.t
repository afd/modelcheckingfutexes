...
{	Trans *T;
	...
	T = trans[ 2][7] = settr(119,2,0,0,0,"ATOMIC", 0, 2, 0);
	...
	trans[2][6]	= settr(118,0,8,8,8,"(run Monitor())", 0, 2, 0);
	trans[2][8]	= settr(120,0,0,9,9,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(109,4,4,10,10,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(112,0,0,11,11,"-end-", 0, 3500, 0);
	...
	T = T->nxt	= settr(105,0,59,0,0,"DO", 0, 2, 0);
	    T->nxt	= settr(105,0,104,0,0,"DO", 0, 2, 0);
...
/*->*/	trans[0][3]	= settr(2,34,10,12,12,"D_STEP", 1, 2, 0);
	...
	trans[0][13]	= settr(12,4,42,16,16,"((old_value==2))", 1, 2, 0); /* m: 26 -> 42,0 */
	reached0[26] = 1;
	trans[0][26]	= settr(25,0,42,17,17,".(goto)", 1, 2, 0);
	trans[0][14]	= settr(13,2,15,2,0,"else", 1, 2, 0);
	trans[0][15]	= settr(14,2,19,18,0,"assert((old_value==1))", 1, 2, 0);
	T = trans[ 0][19] = settr(18,0,0,0,0,"sub-sequence", 1, 2, 0);
	T->nxt	= settr(18,0,18,0,0,"sub-sequence", 1, 2, 0);
/*->*/	trans[0][18]	= settr(17,34,23,19,19,"D_STEP", 1, 2, 0);
	T = trans[0][23] = settr(22,2,0,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(22,2,20,0,0,"IF", 1, 2, 0);
	    T->nxt	= settr(22,2,22,0,0,"IF", 1, 2, 0);
	trans[0][20]	= settr(19,4,54,20,20,"((old_value==0))", 1, 2, 0); /* m: 21 -> 54,0 */
	reached0[21] = 1;
	trans[0][21]	= settr(20,0,54,1,0,"goto retry", 1, 2, 0);
	trans[0][24]	= settr(23,4,42,21,21,".(goto)", 1, 2, 0); /* m: 26 -> 0,42 */
	reached0[26] = 1;
...
/*->*/	trans[0][33]	= settr(32,32,36,22,22,"D_STEP", 1, 2, 0);
/*->*/	trans[0][36]	= settr(35,32,54,23,23,"D_STEP", 1, 2, 0);
	trans[0][41]	= settr(40,0,54,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][39]	= settr(38,32,54,24,24,"D_STEP", 1, 2, 0);
...
/*->*/	trans[0][45]	= settr(44,34,52,25,25,"D_STEP", 1, 2, 0);
	T = trans[0][52] = settr(51,2,0,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(51,2,47,0,0,"IF", 1, 2, 0);
	    T->nxt	= settr(51,2,50,0,0,"IF", 1, 2, 0);
	trans[0][47]	= settr(46,4,58,26,26,"((old_value==0))", 1, 2, 0); /* m: 48 -> 58,0 */
	reached0[48] = 1;
	trans[0][48]	= settr(0,0,0,0,0,"printf('T%d locks mutex on slow path\\n',_pid)",0,0,0);
	trans[0][49]	= settr(48,0,58,1,0,"goto acquired_mutex", 1, 2, 0);
	trans[0][53]	= settr(52,0,55,27,27,".(goto)", 1, 2, 0);
	trans[0][50]	= settr(49,2,51,2,0,"else", 1, 2, 0);
	trans[0][51]	= settr(50,0,55,28,28,"printf('T%d fails to lock mutex on slow path\\n',_pid)", 1, 2, 0); /* m: 53 -> 0,55 */
	reached0[53] = 1;
	trans[0][57]	= settr(56,0,58,1,0,"break", 0, 2, 0);
	trans[0][58]	= settr(57,0,60,1,0,"(1)", 0, 2, 0);
	trans[0][60]	= settr(59,0,61,29,29,"num_threads_in_cs = (num_threads_in_cs+1)", 1, 2, 0);
	trans[0][61]	= settr(60,0,103,30,30,"num_threads_in_cs = (num_threads_in_cs-1)", 1, 2, 0);
	T = trans[ 0][103] = settr(102,0,0,0,0,"sub-sequence", 0, 2, 0);
	T->nxt	= settr(102,0,67,0,0,"sub-sequence", 0, 2, 0);
/*->*/	trans[0][67]	= settr(66,32,101,31,31,"D_STEP", 1, 2, 0);
	T = trans[0][101] = settr(100,0,0,0,0,"IF", 0, 2, 0);
	T = T->nxt	= settr(100,0,71,0,0,"IF", 0, 2, 0);
	    T->nxt	= settr(100,0,100,0,0,"IF", 0, 2, 0);
/*->*/	trans[0][71]	= settr(70,32,97,32,32,"D_STEP", 1, 2, 0);
	...
	T->nxt	= settr(95,2,72,0,0,"ATOMIC", 1, 2, 0);
	trans[0][72]	= settr(71,2,91,33,33,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 73 -> 0,91 */
	reached0[73] = 1;
	...
	T = T->nxt	= settr(90,2,74,0,0,"DO", 1, 2, 0);
	    T->nxt	= settr(90,2,76,0,0,"DO", 1, 2, 0);
	trans[0][74]	= settr(73,0,106,34,34,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 94 -> 106,0 */
	reached0[94] = 1;
	trans[0][75]	= settr(74,2,94,1,0,"goto :b2", 1, 2, 0); /* m: 94 -> 0,106 */
	reached0[94] = 1;
	trans[0][76]	= settr(75,2,87,2,0,"else", 1, 2, 0);
	T = trans[0][87] = settr(86,2,0,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(86,2,77,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(86,2,79,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(86,2,81,0,0,"IF", 1, 2, 0);
	T = T->nxt	= settr(86,2,83,0,0,"IF", 1, 2, 0);
	    T->nxt	= settr(86,2,85,0,0,"IF", 1, 2, 0);
	trans[0][77]	= settr(76,2,91,35,35,"(futex.wait[1])", 1, 2, 0); /* m: 78 -> 91,0 */
	reached0[78] = 1;
	trans[0][78]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][88]	= settr(87,2,89,1,0,".(goto)", 1, 2, 0); /* m: 89 -> 0,91 */
	reached0[89] = 1;
	trans[0][79]	= settr(78,2,91,36,36,"(futex.wait[2])", 1, 2, 0); /* m: 80 -> 91,0 */
	reached0[80] = 1;
	trans[0][80]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][81]	= settr(80,2,91,37,37,"(futex.wait[3])", 1, 2, 0); /* m: 82 -> 91,0 */
	reached0[82] = 1;
	trans[0][82]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][83]	= settr(82,2,91,38,38,"(futex.wait[4])", 1, 2, 0); /* m: 84 -> 91,0 */
	reached0[84] = 1;
	trans[0][84]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][85]	= settr(84,2,91,39,39,"(futex.wait[5])", 1, 2, 0); /* m: 86 -> 91,0 */
	reached0[86] = 1;
	trans[0][86]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][89]	= settr(88,2,91,40,40,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 90 -> 0,91 */
	reached0[90] = 1;
	trans[0][90]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][93]	= settr(92,2,94,1,0,"break", 1, 2, 0);
	trans[0][94]	= settr(93,0,106,41,41,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 95 -> 0,106 */
	reached0[95] = 1;
	trans[0][95]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][102]	= settr(101,0,106,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][100]	= settr(99,32,106,42,42,"D_STEP", 0, 2, 0);
	trans[0][104]	= settr(103,0,109,43,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][105]	= settr(104,0,109,1,0,"goto :b0", 0, 2, 0);
	trans[0][108]	= settr(107,0,109,1,0,"break", 0, 2, 0);
	trans[0][109]	= settr(108,0,0,44,44,"-end-", 0, 3500, 0);
	/* np_ demon: */
