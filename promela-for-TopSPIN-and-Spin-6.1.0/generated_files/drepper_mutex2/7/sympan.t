...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(118,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(119,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(120,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(121,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= /* c */ settr(122,2,7,3,3,"(run Thread())", 0, 2, 0);
	trans[2][7]	= /* c */ settr(123,2,8,3,3,"(run Thread())", 0, 2, 0);
	trans[2][8]	= settr(124,0,10,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][10]	= settr(126,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(113,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(116,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][71]	= settr(70,32,101,28,28,"D_STEP", 1, 2, 0);
	...
	trans[0][72]	= settr(71,2,95,29,29,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 73 -> 0,95 */
	reached0[73] = 1;
	...
	trans[0][74]	= settr(73,0,110,30,30,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 98 -> 110,0 */
	reached0[98] = 1;
	...
	trans[0][77]	= settr(76,2,95,31,31,"(futex.wait[1])", 1, 2, 0); /* m: 78 -> 95,0 */
	reached0[78] = 1;
	trans[0][78]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][92]	= settr(91,2,93,1,0,".(goto)", 1, 2, 0); /* m: 93 -> 0,95 */
	reached0[93] = 1;
	trans[0][79]	= settr(78,2,95,32,32,"(futex.wait[2])", 1, 2, 0); /* m: 80 -> 95,0 */
	reached0[80] = 1;
	trans[0][80]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][81]	= settr(80,2,95,33,33,"(futex.wait[3])", 1, 2, 0); /* m: 82 -> 95,0 */
	reached0[82] = 1;
	trans[0][82]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][83]	= settr(82,2,95,34,34,"(futex.wait[4])", 1, 2, 0); /* m: 84 -> 95,0 */
	reached0[84] = 1;
	trans[0][84]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][85]	= settr(84,2,95,35,35,"(futex.wait[5])", 1, 2, 0); /* m: 86 -> 95,0 */
	reached0[86] = 1;
	trans[0][86]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][87]	= settr(86,2,95,36,36,"(futex.wait[6])", 1, 2, 0); /* m: 88 -> 95,0 */
	reached0[88] = 1;
	trans[0][88]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][89]	= settr(88,2,95,37,37,"(futex.wait[7])", 1, 2, 0); /* m: 90 -> 95,0 */
	reached0[90] = 1;
	trans[0][90]	= settr(0,0,0,0,0,"futex.wait[7] = 0",0,0,0);
	trans[0][93]	= settr(92,2,95,38,38,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 94 -> 0,95 */
	reached0[94] = 1;
	trans[0][94]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][97]	= settr(96,2,98,1,0,"break", 1, 2, 0);
	trans[0][98]	= settr(97,0,110,39,39,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 99 -> 0,110 */
	reached0[99] = 1;
	trans[0][99]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][106]	= settr(105,0,110,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][104]	= settr(103,32,110,40,40,"D_STEP", 0, 2, 0);
	trans[0][108]	= settr(107,0,113,41,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][109]	= settr(108,0,113,1,0,"goto :b0", 0, 2, 0);
	trans[0][112]	= settr(111,0,113,1,0,"break", 0, 2, 0);
	trans[0][113]	= settr(112,0,0,42,42,"-end-", 0, 3500, 0);
	/* np_ demon: */
