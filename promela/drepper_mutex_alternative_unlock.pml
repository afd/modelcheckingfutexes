 ...
 *   spin -DNUM_THREADS=3 -search -ltl safe_cs -m100000 drepper_mutex2.pml
 *
 * Test with 3 threads, checking for invalid end states:
 *   spin -DNUM_THREADS=3 -search -noclaim -m100000 drepper_mutex2.pml
 */
...
inline lock() {
  ...
  atomic {
    ...
    if
    :: old_value == 0 -> printf("T%d locks mutex on fast path\n", _pid);
       goto acquired_mutex
...
retry:
     atomic {
       ...
       if
       :: old_value == 0 -> printf("T%d locks mutex on slow path\n", _pid);
          goto acquired_mutex
       :: else -> printf("T%d fails to lock mutex on slow path\n", _pid)
       fi
     }
  od
  acquired_mutex:
...
inline unlock() {
  ...
  d_step {
    ...
    printf("T%d exchangs futex word from %d to %d\n", _pid, old_value, futex.word);
  }
  if
