...
/*** From source: ../../../gustedt_mutex2.pml ***/
...
#define Index(x, y)	Boundcheck(x, y, 0, 0, 0)
	...
	case 0:	/* Thread */
		((P0 *)pptr(h))->_t = 0;
		((P0 *)pptr(h))->_p = 117; reached0[117]=1;
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->num_woken = 0;
		((P0 *)pptr(h))->prev_lock = 0;
		((P0 *)pptr(h))->cur = 0;
		((P0 *)pptr(h))->prev_unlock = 0;
#ifdef VAR_RANGES
		logval("Thread:num_woken", ((P0 *)pptr(h))->num_woken);
		logval("Thread:prev_lock", ((P0 *)pptr(h))->prev_lock);
		logval("Thread:cur", ((P0 *)pptr(h))->cur);
		logval("Thread:prev_unlock", ((P0 *)pptr(h))->prev_unlock);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{	/* int i; */
	...
	printf("	byte   prev_lock:	%d\n", ((P0 *)pptr(pid))->prev_lock);
	printf("	byte   cur:	%d\n", ((P0 *)pptr(pid))->cur);
	printf("	byte   prev_unlock:	%d\n", ((P0 *)pptr(pid))->prev_unlock);
		break;
	}
}
void
