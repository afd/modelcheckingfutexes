...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(104,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(105,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(106,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(107,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= settr(108,0,8,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][8]	= settr(110,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(99,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(102,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][61]	= settr(60,32,87,23,23,"D_STEP", 0, 2, 0);
	...
	trans[0][62]	= settr(61,2,81,24,24,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 63 -> 0,81 */
	reached0[63] = 1;
	...
	trans[0][64]	= settr(63,0,96,25,25,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 84 -> 96,0 */
	reached0[84] = 1;
	...
	trans[0][67]	= settr(66,2,81,26,26,"(futex.wait[1])", 1, 2, 0); /* m: 68 -> 81,0 */
	reached0[68] = 1;
	trans[0][68]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][78]	= settr(77,2,79,1,0,".(goto)", 1, 2, 0); /* m: 79 -> 0,81 */
	reached0[79] = 1;
	trans[0][69]	= settr(68,2,81,27,27,"(futex.wait[2])", 1, 2, 0); /* m: 70 -> 81,0 */
	reached0[70] = 1;
	trans[0][70]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][71]	= settr(70,2,81,28,28,"(futex.wait[3])", 1, 2, 0); /* m: 72 -> 81,0 */
	reached0[72] = 1;
	trans[0][72]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][73]	= settr(72,2,81,29,29,"(futex.wait[4])", 1, 2, 0); /* m: 74 -> 81,0 */
	reached0[74] = 1;
	trans[0][74]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][75]	= settr(74,2,81,30,30,"(futex.wait[5])", 1, 2, 0); /* m: 76 -> 81,0 */
	reached0[76] = 1;
	trans[0][76]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][79]	= settr(78,2,81,31,31,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 80 -> 0,81 */
	reached0[80] = 1;
	trans[0][80]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][83]	= settr(82,2,84,1,0,"break", 1, 2, 0);
	trans[0][84]	= settr(83,0,96,32,32,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 85 -> 0,96 */
	reached0[85] = 1;
	trans[0][85]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][92]	= settr(91,0,96,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][90]	= settr(89,32,96,33,33,"D_STEP", 0, 2, 0);
	trans[0][94]	= settr(93,0,99,34,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][95]	= settr(94,0,99,1,0,"goto :b0", 0, 2, 0);
	trans[0][98]	= settr(97,0,99,1,0,"break", 0, 2, 0);
	trans[0][99]	= settr(98,0,0,35,35,"-end-", 0, 3500, 0);
	/* np_ demon: */
