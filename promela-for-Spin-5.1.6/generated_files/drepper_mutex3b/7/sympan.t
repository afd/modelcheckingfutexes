...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(108,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(109,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(110,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(111,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= /* c */ settr(112,2,7,3,3,"(run Thread())", 0, 2, 0);
	trans[2][7]	= /* c */ settr(113,2,8,3,3,"(run Thread())", 0, 2, 0);
	trans[2][8]	= settr(114,0,10,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][10]	= settr(116,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(103,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(106,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][61]	= settr(60,32,91,23,23,"D_STEP", 0, 2, 0);
	...
	trans[0][62]	= settr(61,2,85,24,24,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 63 -> 0,85 */
	reached0[63] = 1;
	...
	trans[0][64]	= settr(63,0,100,25,25,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 88 -> 100,0 */
	reached0[88] = 1;
	...
	trans[0][67]	= settr(66,2,85,26,26,"(futex.wait[1])", 1, 2, 0); /* m: 68 -> 85,0 */
	reached0[68] = 1;
	trans[0][68]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][82]	= settr(81,2,83,1,0,".(goto)", 1, 2, 0); /* m: 83 -> 0,85 */
	reached0[83] = 1;
	trans[0][69]	= settr(68,2,85,27,27,"(futex.wait[2])", 1, 2, 0); /* m: 70 -> 85,0 */
	reached0[70] = 1;
	trans[0][70]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][71]	= settr(70,2,85,28,28,"(futex.wait[3])", 1, 2, 0); /* m: 72 -> 85,0 */
	reached0[72] = 1;
	trans[0][72]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][73]	= settr(72,2,85,29,29,"(futex.wait[4])", 1, 2, 0); /* m: 74 -> 85,0 */
	reached0[74] = 1;
	trans[0][74]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][75]	= settr(74,2,85,30,30,"(futex.wait[5])", 1, 2, 0); /* m: 76 -> 85,0 */
	reached0[76] = 1;
	trans[0][76]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][77]	= settr(76,2,85,31,31,"(futex.wait[6])", 1, 2, 0); /* m: 78 -> 85,0 */
	reached0[78] = 1;
	trans[0][78]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][79]	= settr(78,2,85,32,32,"(futex.wait[7])", 1, 2, 0); /* m: 80 -> 85,0 */
	reached0[80] = 1;
	trans[0][80]	= settr(0,0,0,0,0,"futex.wait[7] = 0",0,0,0);
	trans[0][83]	= settr(82,2,85,33,33,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 84 -> 0,85 */
	reached0[84] = 1;
	trans[0][84]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][87]	= settr(86,2,88,1,0,"break", 1, 2, 0);
	trans[0][88]	= settr(87,0,100,34,34,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 89 -> 0,100 */
	reached0[89] = 1;
	trans[0][89]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][96]	= settr(95,0,100,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][94]	= settr(93,32,100,35,35,"D_STEP", 0, 2, 0);
	trans[0][98]	= settr(97,0,103,36,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][99]	= settr(98,0,103,1,0,"goto :b0", 0, 2, 0);
	trans[0][102]	= settr(101,0,103,1,0,"break", 0, 2, 0);
	trans[0][103]	= settr(102,0,0,37,37,"-end-", 0, 3500, 0);
	/* np_ demon: */
