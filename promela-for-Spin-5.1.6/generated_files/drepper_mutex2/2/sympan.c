...
/*** From source: ../../../drepper_mutex2.pml ***/
...
State *rep(State *orig_now) {
   ...
   memcpy(&min_now,orig_now, vsize);
   {
   State originalForThisStrategy;
   memcpy(&originalForThisStrategy,&min_now,vsize);

   int i0;

   State partialImages[1];

   for(i0=0; (i0<2); i0++) {
      memcpy(&partialImages[0],&originalForThisStrategy,vsize);
      applyPermToState(&partialImages[0],&elementset_1[0][i0]);

      if(memcmp(&partialImages[0],&min_now,vsize)<0) {
         memcpy(&min_now,&partialImages[0],vsize);
      }
}
   } /* End of sims enumeration */
   return &min_now;
}
...
#else
#define onstack_zap()	gstore((char *) rep(&now), vsize, 4)
#endif
...
#define Index(x, y)	Boundcheck(x, y, 0, 0, 0)
	...
	case 0:	/* Thread */
		((P0 *)pptr(h))->_t = 0;
		((P0 *)pptr(h))->_p = 100; reached0[100]=1;
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->num_woken = 0;
		((P0 *)pptr(h))->old_value_lock = 0;
		((P0 *)pptr(h))->old_value_unlock = 0;
#ifdef VAR_RANGES
		logval("Thread:num_woken", ((P0 *)pptr(h))->num_woken);
		logval("Thread:old_value_lock", ((P0 *)pptr(h))->old_value_lock);
		logval("Thread:old_value_unlock", ((P0 *)pptr(h))->old_value_unlock);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
#ifdef BITSTATE
			if (!bstore((char *)rep(&now), vsize))
#else
#ifdef MA
			if (!gstore((char *)rep(&now), vsize, 0))
#else
			if (!hstore((char *)rep(&now), vsize))
#endif
#endif
...
#else
			II = gstore((char *)rep(&now), vsize, 0);
#endif
...
iniglobals(void)
{
		now.num_threads_in_cs = 0;
		now.futex.word = 0;
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			now.futex.wait[l_in] = 0;
		}
	}
		now.futex.num_waiting = 0;
#ifdef VAR_RANGES
		logval("num_threads_in_cs", now.num_threads_in_cs);
		logval("futex.word", now.futex.word);
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			logval("futex.wait[l_in]", now.futex.wait[l_in]);
		}
	}
		logval("futex.num_waiting", now.futex.num_waiting);
#endif
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			printf("	bit    wait[%d]:	%d\n", l_in, now.futex.wait[l_in]);
		}
	}
	printf("	byte   num_waiting:	%d\n", now.futex.num_waiting);
	printf("	byte   num_threads_in_cs:	%d\n", now.num_threads_in_cs);
}
void
...
{	/* int i; */
	...
	case 2:
		...
		break;
	case 1:
		printf("local vars proc %d (Monitor):\n", pid);
		break;
	case 0:
		printf("local vars proc %d (Thread):\n", pid);
	printf("	byte   num_woken:	%d\n", ((P0 *)pptr(pid))->num_woken);
	printf("	byte   old_value_lock:	%d\n", ((P0 *)pptr(pid))->old_value_lock);
	printf("	byte   old_value_unlock:	%d\n", ((P0 *)pptr(pid))->old_value_unlock);
		break;
	}
}
void
