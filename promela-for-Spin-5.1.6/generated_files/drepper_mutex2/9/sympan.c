...
State *rep(State *orig_now) {
   ...
   {
      int j;
      State current_min, tmp_now;
      do {
         memcpy(&current_min,&min_now,vsize);

         for(j=0; j<36; j++) {
            memcpy(&tmp_now,&min_now,vsize);
            applyPermToState(&tmp_now,&(elementset_1[j]));
            if(memcmp(&tmp_now,&min_now,vsize)<0) {
               memcpy(&min_now,&tmp_now,vsize);
            }
         }
      } while(memcmp(&min_now,&current_min,vsize)!=0);
...
#define Index(x, y)	Boundcheck(x, y, 0, 0, 0)
	...
	case 0:	/* Thread */
		((P0 *)pptr(h))->_t = 0;
		((P0 *)pptr(h))->_p = 114; reached0[114]=1;
		/* params: */
		/* locals: */
