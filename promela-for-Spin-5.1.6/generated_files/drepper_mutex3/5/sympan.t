...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(105,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(106,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(107,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(108,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= settr(109,0,8,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][8]	= settr(111,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(100,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(103,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][62]	= settr(61,32,88,23,23,"D_STEP", 1, 2, 0);
	...
	trans[0][63]	= settr(62,2,82,24,24,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 64 -> 0,82 */
	reached0[64] = 1;
	...
	trans[0][65]	= settr(64,0,97,25,25,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 85 -> 97,0 */
	reached0[85] = 1;
	...
	trans[0][68]	= settr(67,2,82,26,26,"(futex.wait[1])", 1, 2, 0); /* m: 69 -> 82,0 */
	reached0[69] = 1;
	trans[0][69]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][79]	= settr(78,2,80,1,0,".(goto)", 1, 2, 0); /* m: 80 -> 0,82 */
	reached0[80] = 1;
	trans[0][70]	= settr(69,2,82,27,27,"(futex.wait[2])", 1, 2, 0); /* m: 71 -> 82,0 */
	reached0[71] = 1;
	trans[0][71]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][72]	= settr(71,2,82,28,28,"(futex.wait[3])", 1, 2, 0); /* m: 73 -> 82,0 */
	reached0[73] = 1;
	trans[0][73]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][74]	= settr(73,2,82,29,29,"(futex.wait[4])", 1, 2, 0); /* m: 75 -> 82,0 */
	reached0[75] = 1;
	trans[0][75]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][76]	= settr(75,2,82,30,30,"(futex.wait[5])", 1, 2, 0); /* m: 77 -> 82,0 */
	reached0[77] = 1;
	trans[0][77]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][80]	= settr(79,2,82,31,31,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 81 -> 0,82 */
	reached0[81] = 1;
	trans[0][81]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][84]	= settr(83,2,85,1,0,"break", 1, 2, 0);
	trans[0][85]	= settr(84,0,97,32,32,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 86 -> 0,97 */
	reached0[86] = 1;
	trans[0][86]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][93]	= settr(92,0,97,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][91]	= settr(90,32,97,33,33,"D_STEP", 0, 2, 0);
	trans[0][95]	= settr(94,0,100,34,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][96]	= settr(95,0,100,1,0,"goto :b0", 0, 2, 0);
	trans[0][99]	= settr(98,0,100,1,0,"break", 0, 2, 0);
	trans[0][100]	= settr(99,0,0,35,35,"-end-", 0, 3500, 0);
	/* np_ demon: */
