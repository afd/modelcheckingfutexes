...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(109,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(110,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(111,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(112,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= /* c */ settr(113,2,7,3,3,"(run Thread())", 0, 2, 0);
	trans[2][7]	= /* c */ settr(114,2,8,3,3,"(run Thread())", 0, 2, 0);
	trans[2][8]	= settr(115,0,10,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][10]	= settr(117,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(104,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(107,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][62]	= settr(61,32,92,23,23,"D_STEP", 1, 2, 0);
	...
	trans[0][63]	= settr(62,2,86,24,24,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 64 -> 0,86 */
	reached0[64] = 1;
	...
	trans[0][65]	= settr(64,0,101,25,25,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 89 -> 101,0 */
	reached0[89] = 1;
	...
	trans[0][68]	= settr(67,2,86,26,26,"(futex.wait[1])", 1, 2, 0); /* m: 69 -> 86,0 */
	reached0[69] = 1;
	trans[0][69]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][83]	= settr(82,2,84,1,0,".(goto)", 1, 2, 0); /* m: 84 -> 0,86 */
	reached0[84] = 1;
	trans[0][70]	= settr(69,2,86,27,27,"(futex.wait[2])", 1, 2, 0); /* m: 71 -> 86,0 */
	reached0[71] = 1;
	trans[0][71]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][72]	= settr(71,2,86,28,28,"(futex.wait[3])", 1, 2, 0); /* m: 73 -> 86,0 */
	reached0[73] = 1;
	trans[0][73]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][74]	= settr(73,2,86,29,29,"(futex.wait[4])", 1, 2, 0); /* m: 75 -> 86,0 */
	reached0[75] = 1;
	trans[0][75]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][76]	= settr(75,2,86,30,30,"(futex.wait[5])", 1, 2, 0); /* m: 77 -> 86,0 */
	reached0[77] = 1;
	trans[0][77]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][78]	= settr(77,2,86,31,31,"(futex.wait[6])", 1, 2, 0); /* m: 79 -> 86,0 */
	reached0[79] = 1;
	trans[0][79]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][80]	= settr(79,2,86,32,32,"(futex.wait[7])", 1, 2, 0); /* m: 81 -> 86,0 */
	reached0[81] = 1;
	trans[0][81]	= settr(0,0,0,0,0,"futex.wait[7] = 0",0,0,0);
	trans[0][84]	= settr(83,2,86,33,33,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 85 -> 0,86 */
	reached0[85] = 1;
	trans[0][85]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][88]	= settr(87,2,89,1,0,"break", 1, 2, 0);
	trans[0][89]	= settr(88,0,101,34,34,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 90 -> 0,101 */
	reached0[90] = 1;
	trans[0][90]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][97]	= settr(96,0,101,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][95]	= settr(94,32,101,35,35,"D_STEP", 0, 2, 0);
	trans[0][99]	= settr(98,0,104,36,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][100]	= settr(99,0,104,1,0,"goto :b0", 0, 2, 0);
	trans[0][103]	= settr(102,0,104,1,0,"break", 0, 2, 0);
	trans[0][104]	= settr(103,0,0,37,37,"-end-", 0, 3500, 0);
	/* np_ demon: */
