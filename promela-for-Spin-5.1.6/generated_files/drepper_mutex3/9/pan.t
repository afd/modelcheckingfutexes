...
{	Trans *T;
	...
	trans[2][2]	= /* c */ settr(113,2,3,3,3,"(run Thread())", 0, 2, 0);
	trans[2][3]	= /* c */ settr(114,2,4,3,3,"(run Thread())", 0, 2, 0);
	trans[2][4]	= /* c */ settr(115,2,5,3,3,"(run Thread())", 0, 2, 0);
	trans[2][5]	= /* c */ settr(116,2,6,3,3,"(run Thread())", 0, 2, 0);
	trans[2][6]	= /* c */ settr(117,2,7,3,3,"(run Thread())", 0, 2, 0);
	trans[2][7]	= /* c */ settr(118,2,8,3,3,"(run Thread())", 0, 2, 0);
	trans[2][8]	= /* c */ settr(119,2,9,3,3,"(run Thread())", 0, 2, 0);
	trans[2][9]	= /* c */ settr(120,2,10,3,3,"(run Thread())", 0, 2, 0);
	trans[2][10]	= settr(121,0,12,4,4,"(run Monitor())", 0, 2, 0);
	trans[2][12]	= settr(123,0,0,5,5,"-end-", 0, 3500, 0);
	...
	trans[1][1]	= settr(108,4,4,6,6,"((num_threads_in_cs>1))", 1, 2, 0); /* m: 2 -> 4,0 */
	reached1[2] = 1;
	trans[1][2]	= settr(0,0,0,0,0,"assert(0)",0,0,0);
	trans[1][4]	= settr(111,0,0,7,7,"-end-", 0, 3500, 0);
...
/*->*/	trans[0][62]	= settr(61,32,96,23,23,"D_STEP", 1, 2, 0);
	...
	trans[0][63]	= settr(62,2,90,24,24,"assert(!(futex.wait[_pid]))", 1, 2, 0); /* m: 64 -> 0,90 */
	reached0[64] = 1;
	...
	trans[0][65]	= settr(64,0,105,25,25,"(((num_woken==1)||(futex.num_waiting==0)))", 1, 2, 0); /* m: 93 -> 105,0 */
	reached0[93] = 1;
	...
	trans[0][68]	= settr(67,2,90,26,26,"(futex.wait[1])", 1, 2, 0); /* m: 69 -> 90,0 */
	reached0[69] = 1;
	trans[0][69]	= settr(0,0,0,0,0,"futex.wait[1] = 0",0,0,0);
	trans[0][87]	= settr(86,2,88,1,0,".(goto)", 1, 2, 0); /* m: 88 -> 0,90 */
	reached0[88] = 1;
	trans[0][70]	= settr(69,2,90,27,27,"(futex.wait[2])", 1, 2, 0); /* m: 71 -> 90,0 */
	reached0[71] = 1;
	trans[0][71]	= settr(0,0,0,0,0,"futex.wait[2] = 0",0,0,0);
	trans[0][72]	= settr(71,2,90,28,28,"(futex.wait[3])", 1, 2, 0); /* m: 73 -> 90,0 */
	reached0[73] = 1;
	trans[0][73]	= settr(0,0,0,0,0,"futex.wait[3] = 0",0,0,0);
	trans[0][74]	= settr(73,2,90,29,29,"(futex.wait[4])", 1, 2, 0); /* m: 75 -> 90,0 */
	reached0[75] = 1;
	trans[0][75]	= settr(0,0,0,0,0,"futex.wait[4] = 0",0,0,0);
	trans[0][76]	= settr(75,2,90,30,30,"(futex.wait[5])", 1, 2, 0); /* m: 77 -> 90,0 */
	reached0[77] = 1;
	trans[0][77]	= settr(0,0,0,0,0,"futex.wait[5] = 0",0,0,0);
	trans[0][78]	= settr(77,2,90,31,31,"(futex.wait[6])", 1, 2, 0); /* m: 79 -> 90,0 */
	reached0[79] = 1;
	trans[0][79]	= settr(0,0,0,0,0,"futex.wait[6] = 0",0,0,0);
	trans[0][80]	= settr(79,2,90,32,32,"(futex.wait[7])", 1, 2, 0); /* m: 81 -> 90,0 */
	reached0[81] = 1;
	trans[0][81]	= settr(0,0,0,0,0,"futex.wait[7] = 0",0,0,0);
	trans[0][82]	= settr(81,2,90,33,33,"(futex.wait[8])", 1, 2, 0); /* m: 83 -> 90,0 */
	reached0[83] = 1;
	trans[0][83]	= settr(0,0,0,0,0,"futex.wait[8] = 0",0,0,0);
	trans[0][84]	= settr(83,2,90,34,34,"(futex.wait[9])", 1, 2, 0); /* m: 85 -> 90,0 */
	reached0[85] = 1;
	trans[0][85]	= settr(0,0,0,0,0,"futex.wait[9] = 0",0,0,0);
	trans[0][88]	= settr(87,2,90,35,35,"futex.num_waiting = (futex.num_waiting-1)", 1, 2, 0); /* m: 89 -> 0,90 */
	reached0[89] = 1;
	trans[0][89]	= settr(0,0,0,0,0,"num_woken = (num_woken+1)",0,0,0);
	trans[0][92]	= settr(91,2,93,1,0,"break", 1, 2, 0);
	trans[0][93]	= settr(92,0,105,36,36,"printf('T%d woke up %d thread(s)\\n',_pid,num_woken)", 1, 2, 0); /* m: 94 -> 0,105 */
	reached0[94] = 1;
	trans[0][94]	= settr(0,0,0,0,0,"num_woken = 0",0,0,0);
	trans[0][101]	= settr(100,0,105,1,0,".(goto)", 0, 2, 0);
/*->*/	trans[0][99]	= settr(98,32,105,37,37,"D_STEP", 0, 2, 0);
	trans[0][103]	= settr(102,0,108,38,0,"printf('T%d is done\\n',_pid)", 0, 2, 0);
	trans[0][104]	= settr(103,0,108,1,0,"goto :b0", 0, 2, 0);
	trans[0][107]	= settr(106,0,108,1,0,"break", 0, 2, 0);
	trans[0][108]	= settr(107,0,0,39,39,"-end-", 0, 3500, 0);
	/* np_ demon: */
