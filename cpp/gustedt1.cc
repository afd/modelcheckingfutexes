...
public:
  Mutex() : futex_word(0) {}
  void lock() {
    uint32_t cur = cmpxchg(futex_word, 0, set_lock(1)); /*@\label{line:gustedt1:firstcmpxchg}@*/
  ...
  void unlock() {
    uint32_t prev = futex_word.fetch_sub(set_lock(1)); /*@\label{line:gustedt1:decrement}@*/
    if (prev != set_lock(1)) {
