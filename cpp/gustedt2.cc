...
public:
  Mutex() : futex_word(0) {}
  void lock() {
    uint32_t cur = cmpxchg(futex_word, 0, set_locked(1)); /*@\label{line:gustedt2:firstcmpxchg}@*/
    if (cur == 0) return; /*@\label{line:gustedt2:fastpathreturn}@*/
    for (uint32_t i = 0; i < BUSYWAIT; i++) { /*@\label{line:gustedt2:busywait}@*/
      if (is_locked(cur)) cur = unset_locked(cur) - 1; /*@\label{line:gustedt2:busywaitdec}@*/
      uint32_t prev =
        cmpxchg(futex_word, cur, set_locked(cur+1)); /*@\label{line:gustedt2:busywaitcmpxchg}@*/
      if (prev == cur) return;
      cur = prev;
    }
    ...
    for (;;) { /*@\label{line:gustedt2:unboundedloop}@*/
      ...
      uint32_t prev =
        cmpxchg(futex_word, cur, set_locked(cur)); /*@\label{line:gustedt2:slowpathcmpxchg}@*/
